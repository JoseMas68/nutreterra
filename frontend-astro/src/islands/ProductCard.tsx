...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group transform hover:-translate-y-2 relative">
      {/* Botón de favoritos */}
      <button
      ...
      <a href={`/producto/${product.slug}`} className="block relative overflow-hidden aspect-square">
        ...
        {discountPercentage > 0 && (
          <span className="absolute top-4 left-4 bg-accent text-white px-3 py-1 rounded-full font-bold text-sm shadow-lg">
            -{discountPercentage}%
          </span>
        )}
      </a>
      ...
      <div className="p-6">
        <a href={`/producto/${product.slug}`} className="block">
          ...
          {/* Nombre del producto */}
          <h3 className="text-xl font-bold text-gray-900 mb-2 hover:text-primary transition-colors leading-tight min-h-[3.5rem]">
            {product.name}
          </h3>
          ...
          {/* Tags integrados de forma minimalista */}
        ...
        <div className="flex items-end justify-between pt-4 border-t border-gray-100">
          ...
          <button
            ...
            className={`px-9 py-2.5 rounded-lg font-semibold transition-all flex items-center justify-center gap-2 ${
              product.stock <= 0
